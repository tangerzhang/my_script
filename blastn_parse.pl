...
if ((!defined $opt_i)|| (!defined $opt_o)  || (!defined $opt_q)) {
    die "************************************************************************
    Usage: perl blastn_parse.pl -i input -o output -q query.fasta -b 0||1 -c coverage -d identity 
      -h : help and usage.
      -q : query file, fasta format; to calculate coverage, we need to get sequence length of query
